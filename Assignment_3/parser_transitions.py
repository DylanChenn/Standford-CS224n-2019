...
def minibatch_parse(sentences, model, batch_size):
    ...
    """
    dependencies = list(range(len(sentences)))
    ...
    while len(unfinished_parses) > 0:
        ...
        for un_idx in reversed(range(len(transitions))):
            unfinished_parses[un_idx].parse_step(transitions[un_idx])
            if len(unfinished_parses[un_idx].stack) == 1 and len(unfinished_parses[un_idx].buffer) == 0:
                temp = unfinished_parses.pop(un_idx)
                dependencies[partial_parses.index(temp)] = temp.dependencies
